...
| **Total** | **599** | **574** | **731** | **18.1%** |

The complete AXI migration reduces aggregate legacy-v1 tokens by 18.1%; the median per-task change
is a 27.7% reduction and four of six task shapes improve. The richer execution preview and
recoverable usage error are larger than their legacy responses because v2 adds original byte count,
content classification, valid flags, and a concrete safe next command.

TOON by itself is not universally smaller: the same v2 data as compact JSON is 574 tokens, 4.4%
below TOON's 599. The migration's measured reduction comes from content-first roots, fewer calls,
